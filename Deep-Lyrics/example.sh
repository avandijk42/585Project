...
python ./preprocess.py --input_file $FILE_NAME.txt

# train the LSTM model to fit the lyrics data
python ./train.py --training_file $FILE_NAME.txt --vocabulary_file $FILE_NAME.vocab --model_name $MODEL_NAME

# generate new lyrics and save them in a file
python ./sample.py --model_name $MODEL_NAME --vocabulary_file $FILE_NAME.vocab --output_file sample.txt
